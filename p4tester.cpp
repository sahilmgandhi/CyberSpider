...
{
	//createDB("testBigBibek", 100000);
	cout << "created the data base " << endl;
	//ingest("testBigBibek", "telemetry-50.txt");
	cout << "ingested the file" << endl;
	//purge("testNo1", "purgeFile.txt");
	crawl("testBigBibek", "inidicators1.txt", 10, "outputBigBib.txt");
	//convertToJavaScript("outputBigBibek.txt", "graphtemplate.html", "outputBigBibek.html");

	//purge("SuperSwag", "purgeFile.txt");
	/*
	if (argc < 2 || argv[1][0] != '-')
		printUsageAndExit();
	switch (argv[1][1])
